...
if(isset($gp["button"])) {
		...
		html::alert("error", "Products should not contain special characters.", "Wrong products Input");
			...
			switch($gp['d']['axn']):
				case 'create':
					...
					product_options::insertProductOptions(	$s['user_id'], 
												$gp['d']['option_name'],
				...
				case 'update':		
					...
					product_options::updateProductOptions(	$gp['d']['product_option_id'], 
															$gp['d']['option_name'],
															$gp['d']['option_description'],  
															$thumbnail,
															$small,
	...
	if(strtolower($gp['delete']) == "delete") {
		product_options::deleteProductOptions($gp['d']['product_option_id']);
	}
}
?>